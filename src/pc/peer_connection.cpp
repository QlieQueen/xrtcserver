...
static int parse_ssrc_info(std::vector<SsrcInfo>& ssrc_info, const std::string& line) {
    ...
    // ssrc:<ssrc-id>
    std::string ssrc_id_s = field1.substr(5); //取出ssrc-id
    uint32_t ssrc_id = 0;
...
int PeerConnection::set_remote_sdp(const std::string& sdp) {
    ...
    std::vector<StreamParams> audio_tracks; // 对应webrtc::AudioTrack
    std::vector<StreamParams> video_tracks; // 对应webrtc::VideoTrack
