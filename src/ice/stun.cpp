        ...
        uint16_t attr_size, const char* key, size_t key_len)
{
    ...
    //后续需要访问到mi_attr_ptr中的成员属性，所以保存裸指针，预防add_attribute(std::move(mi_attr_ptr))后，
    //mi_attr_ptr不再拥有所有权，导致后续使用mi_attr_ptr访问成员属性时造成coredump
    ...
    mi_attr_origin_ptr->copy_bytes(hmac, k_stun_message_integrity_size);
...
bool StunMessage::add_fingerprint() {
    ...
    StunUInt32Attribute* fingerprint_attr_origin_ptr = fingerprint_attr_ptr.get();
    ...
    size_t msg_len_for_crc32 = buf.Length() - k_stun_attribute_header_size - 
        fingerprint_attr_origin_ptr->length();
    uint32_t c = rtc::ComputeCrc32(buf.Data(), msg_len_for_crc32);
    fingerprint_attr_origin_ptr->set_value(c ^ STUN_FINGERPRINT_XOR_VALUE);
    return true;
}
...
bool StunAddressAttribute::write(rtc::ByteBufferWriter* buf) {
    ...
    if (STUN_ADDRESS_UNDEF == stun_family) {
        RTC_LOG(LS_WARNING) << "write address attribute error: unknown address";
        return false;
    }
    ...
    switch (_address.family()) {
        case AF_INET:
            v4addr = _address.ipaddr().ipv4_address();
            buf->WriteBytes((const char*)&v4addr, sizeof(v4addr));
            break;
        case AF_INET6:
            v6addr = _address.ipaddr().ipv6_address();
...
bool StunXorAddressAttribute::write(rtc::ByteBufferWriter* buf) {
    ...
    switch (_address.family()) {
        case AF_INET:
            v4addr = xored_ip.ipv4_address();
            buf->WriteBytes((const char*)&v4addr, sizeof(v4addr));
            break;
        case AF_INET6:
            v6addr = xored_ip.ipv6_address();
            buf->WriteBytes((const char*)&v6addr, sizeof(v6addr));
            break;
        default:
            return false;
...
int get_stun_success_response(int req_type) {
    return is_stun_request_type(req_type)
        ? ((req_type & ~k_stun_class_mask) | STUN_CLASS_SUCCESS)
        : -1;
}

int get_stun_error_response(int req_type) {
    return is_stun_request_type(req_type)
        ? ((req_type & ~k_stun_class_mask) | STUN_CLASS_ERROR)
        : -1;
}
